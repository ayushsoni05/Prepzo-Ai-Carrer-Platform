    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className={`absolute z-[9999] min-w-[8rem] overflow-hidden rounded-md border border-border bg-[#0a0c10] text-white p-1 shadow-md shadow-[0_2px_8px_rgba(0,0,0,0.05)] ${positionClasses} ${alignmentClasses} ${className}`}
          role="menu"
...
export function DropdownSeparator({ className = "" }: DropdownSeparatorProps) {
  return (
    <div className={`mx-1 my-1 h-px bg-white/10 ${className}`} role="separator" />
  )
}
