  ...
  if (!currentQuestion && questions.length === 0) {
    return (
      <div className="p-12 text-center border border-white/5 bg-black rounded-[32px] font-rubik">
        ...
        <p className="text-white/30 mb-8 italic">Neural net returned zero results for this configuration.</p>
        <button 
          onClick={() => {setSearchQuery(''); setSelectedLevel(null); setCurrentQuestionIndex(0);}}
    ...
    <div className="w-full space-y-8 animate-in fade-in duration-700 font-rubik">
          ...
          <div className="relative w-full sm:w-80">
            ...
            >
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <span className="truncate">{selectedSubSkill || 'SELECT DOMAIN'}</span>
              </div>
              <ChevronRight size={14} className={`text-white/20 shrink-0 transition-transform ${isDropdownOpen ? 'rotate-90' : ''}`} />
            </button>
          ...
          <div className="flex flex-col items-center gap-8 w-full lg:w-40 shrink-0">
            ...
            <div className="text-center">
              ...
              <p className="text-2xl font-[900] text-white tracking-tighter italic">
                {currentQuestionIndex + 1}<span className="text-sm opacity-20 mx-1">/</span>{questions.length}
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-10">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.5em] text-[#5ed29c]/60 mb-6 flex items-center gap-2 italic">
                <BookOpen size={14} /> {selectedSubSkill}
              </p>
            ...
            <div className="flex flex-wrap gap-2">
              <span className={`px-4 py-2 bg-white/[0.02] border border-white/5 rounded-lg text-[9px] font-black uppercase tracking-widest italic ${
                currentQuestion.difficulty === 'easy' || currentQuestion.difficulty === 'beginner' ? 'text-green-400' :
                currentQuestion.difficulty === 'medium' || currentQuestion.difficulty === 'intermediate' ? 'text-yellow-400' :
                'text-red-400'
              }`}>
              ...
              <AnimatePresence>
                {showAnswer && (
                  ...
                  >
                    ...
                    <p className="text-[16px] md:text-[18px] leading-relaxed text-white/70 font-medium tracking-tight italic whitespace-pre-wrap">
      ...
      <div className="flex items-center justify-center gap-8 text-[9px] font-black uppercase tracking-[0.3em] text-white/20">
        <div className="flex items-center gap-2">
          <Trophy size={14} /> Total Mastery: {questions.length > 0 ? Math.round((currentQuestionIndex / questions.length) * 100) : 0}%
        </div>
