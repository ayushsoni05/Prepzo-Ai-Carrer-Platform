...
import { Home, FileText, Brain, Briefcase, Settings, Lock, Terminal, Sparkles, Users, Gamepad2 } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ active, onNavigate, badgeMap, lockedItems = [] }) => (
  <div className="sidebar-floating-container hidden md:flex w-auto h-[72px] items-center justify-center bg-[#13171d]/90 backdrop-blur-3xl border border-white/5 rounded-full shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] fixed bottom-8 left-1/2 -translate-x-1/2 z-50 px-4 transition-all duration-500 ease-out">
    ...
    <div className="relative flex flex-row items-center gap-1 z-10">
      {[
        ...
        { id: 'game-lobby', icon: Gamepad2, label: 'Play' },
      ].map((item) => {
